    ...
    bind: function(callback, params) {
      ...
      $('.kit-side-fold').off('click').on('click', function() {
        ...
        if (_side.hasClass('kit-sided')) {
          ...
          _doc.find('i.flexible').removeClass('layui-icon-spread-left');
          _doc.find('i.flexible').addClass('layui-icon-shrink-right');
          ...
          _doc.find('i.flexible').removeClass('layui-icon-shrink-right');
          _doc.find('i.flexible').addClass('layui-icon-spread-left');
    ...
    render: function(callback) {
      var that = this,
        ...
        _tpl = [
          ...
          '<dd>',
          '<a href="javascript:;" kit-target data-options="{url:\'{{child.url}}\',icon:\'{{child.icon}}\',title:\'{{child.title}}\',id:\'{{child.id}}\',target:\'{{child.target}}\'}">',
          ...
          '</dl>',
          '{{# }else{ }}',
          '<a href="javascript:;" kit-target data-options="{url:\'{{item.url}}\',icon:\'{{item.icon}}\',title:\'{{item.title}}\',id:\'{{item.id}}\',target:\'{{item.target}}\'}">',
          '{{# if (item.icon.indexOf("fa-") !== -1) { }}',
          '<i class="fa {{item.icon}}" aria-hidden="true"></i>',
          '{{# } else { }}',
          '<i class="layui-icon">{{item.icon}}</i>',
