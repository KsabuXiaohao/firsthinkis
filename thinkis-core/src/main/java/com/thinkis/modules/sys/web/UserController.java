...
public class UserController extends BaseController {
	...
	public String onlineList(User user,HttpServletRequest request, HttpServletResponse response, Model model) {
		Collection<Session> onlineUserList=sessionDAO.getActiveSessions(true);
		List<Session> seeeionList=new ArrayList<Session>(onlineUserList);
		Page<User> page=new Page<User>(request, response);
		user.setPage(page);
		List<User> userList=new ArrayList<User>();
		for (int i=(page.getPageNo()-1)*page.getPageSize();(i<page.getPageNo()*page.getPageSize()-1)&&i<seeeionList.size();i++) {
			userList.add(UserUtils.get((String)seeeionList.get(i).getAttribute("principalId")));
		}
		// 执行分页查询
		page.setList(userList);
		model.addAttribute("page", page);
		return "modules/sys/onlineUserList";
	}
	...
	public Result listAjax(User user, HttpServletRequest request, HttpServletResponse response) {
	...
	@ResponseBody
	public Result save(User user, HttpServletRequest request, Model model, RedirectAttributes redirectAttributes) {
		...
		if (!"true".equals(checkLoginName(user.getOldLoginName(), user.getLoginName()))){
			return ResultGenerator.genFailResult("保存用户" + user.getLoginName() + "失败，登录名已存在");
		}
		...
		return ResultGenerator.genSuccessResult("保存用户" + user.getLoginName() + "成功");
	}

	@RequiresPermissions("sys:user:edit")
	@RequestMapping(value = "delete")
	@ResponseBody
	public Result delete(User user, RedirectAttributes redirectAttributes) {
		if (UserUtils.getUser().getId().equals(user.getId())){
			return ResultGenerator.genFailResult("删除用户失败, 不允许删除当前用户");
		}else if (User.isAdmin(user.getId())){
			return ResultGenerator.genFailResult("删除用户失败, 不允许删除超级管理员用户");
		}
	...
	public Result batchDelete(String selectedIds) {
		...
		for(String id : ids){
			if (UserUtils.getUser().getId().equals(id)){
				return ResultGenerator.genFailResult("批量删除用户失败, 不允许删除当前用户");
			}else if (User.isAdmin(id)){
				return ResultGenerator.genFailResult("批量删除用户失败, 不允许删除超级管理员用户");
			}
			User user = get(id);
			systemService.deleteUser(user);
		}
	...
	@ResponseBody
	public Result repass(String oldPassword, String newPassword, Model model) {
		User user = UserUtils.getUser();
		if (StringUtils.isNotBlank(oldPassword) && StringUtils.isNotBlank(newPassword)){
			if (SystemService.validatePassword(oldPassword, user.getPassword())){
				systemService.updatePasswordById(user.getId(), user.getLoginName(), newPassword);
				...
				model.addAttribute("message", "修改密码失败，旧密码错误");
				return ResultGenerator.genFailResult("修改密码失败，旧密码错误");
			}
		}
		return ResultGenerator.genSuccessResult("修改密码成功");
	}
