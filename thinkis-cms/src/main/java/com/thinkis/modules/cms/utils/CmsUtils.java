 ...
 */
package com.thinkis.modules.cms.utils;
...
public class CmsUtils {
	...
	public static Site getDefaultSite(){
		Site site = new Site();
		site.setDomain(Site.DEFAULT_DOMAIN);
		List<Site> sites = siteService.findList(site);
		if(sites.size()>0){
			return sites.get(0);
		}else{
			return new Site("1");
	 ...
	 * 获得子站点主导航列表
	 * @param siteId 站点编号
	 * @param parentId 根分类id
	 */
	public static List<Category> getSubMainNavList(String siteId,String parentId){
		@SuppressWarnings("unchecked")
		List<Category> mainNavList = (List<Category>)CacheUtils.get(CMS_CACHE, "mainNavList_"+siteId+"_"+parentId);
		if (mainNavList == null|| mainNavList.size()==0){
			Category category = new Category();
			category.setSite(new Site(siteId));
			...
			page = categoryService.find(page, category);
			mainNavList = page.getList();
			CacheUtils.put(CMS_CACHE, "mainNavList_"+siteId+"_"+parentId, mainNavList);
		}
		return mainNavList;
	}
     ...
     * @param parentId 根分类id
     */
    public static List<Category> getMiniAppSubMainNavList(String siteId,String parentId){
        @SuppressWarnings("unchecked")
        List<Category> mainNavList = (List<Category>)CacheUtils.get(CMS_CACHE, "miniApp_mainNavList_"+siteId+"_"+parentId);
        if (mainNavList == null|| mainNavList.size()==0){
            Category category = new Category();
            category.setSite(new Site(siteId));
            category.setParent(new Category(parentId));
            category.setInMenu(Global.SHOW);
            Page<Category> page = new Page<Category>(1, 4);
            page = categoryService.find(page, category);
            mainNavList = page.getList();
            CacheUtils.put(CMS_CACHE, "miniApp_mainNavList_"+siteId+"_"+parentId, mainNavList);
        }
        return mainNavList;
    }
	 ...
	 * @return
	 */
	public static List<Comment> getHotsArticles(String siteId, int number){
		List<Comment> list = commentService.hotsArticles(siteId,number);
		return list;
	}
